function Particle(x, y) {
    ...
    this.quickUpdate = function(factor){// for the Greedy candidate update
        ...
        if(submodularityMode==2){
                nextPosition = new  Point2(this.position.x+constantStepSize*derivatives[0]/factor,this.position.y+constantStepSize*derivatives[1]/factor);    
        }else{
                nextPosition = new  Point2(this.position.x+0.7*constantStepSize*derivatives[0]/factor,this.position.y+0.7*constantStepSize*derivatives[1]/factor);    
        }
    ...
    this.avoidObstacles = function(nextPosition){
        ...
        if(distP2(this.position,nextPosition)>=(senRange)){
            //print("slow");
            nextPosition1 = plusP2(this.position,productP2(normalizeP2(minusP2(nextPosition,this.position)),senRange));
        }
    ...
    this.pullOutFromObstacle = function(){
        ...
        var p = floor(sqrt(sq(senRange/halfStepSize)-1));
    ...
    this.showShadow = function(particleNum){
        ...
        if(submodularityMode!=2 && submodularityMode!=2.5){
            noStroke();
            fill(244, 66, 66);
    ...
    this.neighbor = function() {//old code 
        ...
        for (var i = 0; i < particles.length; i++) {
            if(i!=this.id){
            	if (distP2(this.position,particles[i].position)<2*senRange){
                    neighbors.push(i);
                }
            }
        }
    ...
    //for custom event density function
    this.sensingModelFunction2 = function(interestedPoint) {
        var p;
        var dis = distP2(this.position,interestedPoint);
        var actualSensingRange = savedParameters[0];
        if (dis > actualSensingRange) {//out of range
            p = 0;
        }
        else if(isLineOfSight(this.position,interestedPoint)==false){
            p = 0; //not in line of sight
        ...
            p = 100*Math.exp(-sensingDecayFactor*dis);
            //print("this is it");
    ...
    this.getEffectiveNeighbors = function(){
        ...
        for(var j1 = 0; j1<tentativeNeighbors.length; j1++){
            ...
            // sensitiveRange < 2R-d
            if((2*senRange - distP2(this.position,particleShadows[j].position))>=sensitiveRange){
       ...
       // no isolations possible
        ...
        for (var x = this.position.x-halfStepSize*p; x<=this.position.x+halfStepSize*p; x+=stepSize){
            for(var y = this.position.y-halfStepSize*p; y<=this.position.y+halfStepSize*p; y+=stepSize){
                    ...
                    if(eventDensity > 0 && disti < senRange && distj < senRange){//intersecting point!!!
    ...
    this.localObjectiveFunctionWRTNeighbors = function(setOfNeighbors){
        ...
        for (var x = this.position.x-halfStepSize*p; x<=this.position.x+halfStepSize*p; x+=stepSize){
            for(var y = this.position.y-halfStepSize*p; y<=this.position.y+halfStepSize*p; y+=stepSize){
                ...
                if(eventDensity>0 && dist<senRange){
                    //ellipse(x,y,2,2);
                    objectiveValueLocal = objectiveValueLocal + (1-this.detectionProbabilityWRTNeighbors(interestedPoint,true,setOfNeighbors))*this.sensingModelFunction(interestedPoint)*eventDensity*areaFactor;
                }
            }
        }
        //print(objectiveValue);
    ...
    this.getDerivatives = function(){//local yet exaustive maybe
        ...
        for (var x = this.position.x-halfStepSize*p; x<=this.position.x+halfStepSize*p; x+=stepSize){
            for(var y = this.position.y-halfStepSize*p; y<=this.position.y+halfStepSize*p; y+=stepSize){
                ...
                var interestedPoint = new Point2(x,y);
                var eventDensity = getEventDensity(interestedPoint);
                if(submodularityMode==2.5){
                    eventDensity = getEventDensity2(interestedPoint);// more events near svaedParticles
                }
                var dist = distP2(this.position,interestedPoint);
                
                if( eventDensity>0 && dist<senRange && dist!=0){
                	//ellipse(x,y,2,2);
        ...
        var deltaTheta = stepSize/senRange;
        ...
        if(arcSections.length==0){
            ...
            for(var j = 0; j<arcSections.length; j++){// considering each arc section
                ...
                while(theta <= arcSections[j][1]){
                    var interestedPoint = new Point2(this.position.x + senRange*cos(theta), this.position.y + senRange*sin(theta)); 
        ...
        for (var x = this.position.x-halfStepSize*p; x<=this.position.x+halfStepSize*p; x+=stepSize){
            for(var y = this.position.y-halfStepSize*p; y<=this.position.y+halfStepSize*p; y+=stepSize){
                ...
                if( eventDensity>0 && dist<senRange && dist!=0){
                    ...
                    constantTermW1 = eventDensity*(1-this.detectionProbability(interestedPoint,true))*(-1*sensingDecayFactor)*this.sensingModelFunction(interestedPoint)*areaFactor;
        ...
        if(arcSections.length==0){
            ...
            if((this.isBoostingActivated==1||boostingActivated==1) && boostingMethod==6){
                ...
                for(var j = 0; j<arcSections.length; j++){// considering each arc section
                    ...
                    while(theta <= arcSections[j][1]){
                        var interestedPoint = new Point2(this.position.x + (senRange-2)*cos(theta), this.position.y + (senRange-2)*sin(theta)); 
            ...
            for(var j = 0; j<arcSections.length; j++){// considering each arc section
                ...
                while(theta <= arcSections[j][1]){
                    var interestedPoint = new Point2(this.position.x + (senRange-1)*cos(theta), this.position.y + (senRange-1)*sin(theta)); 
    ...
    this.boostingBeta3 = function(interestedPoint){
        ...
        return ((1-Math.exp(-1*sensingDecayFactor*senRange/boostingParameterY)*(1+senRange*sensingDecayFactor/boostingParameterY))/(sensingDecayFactor/boostingParameterY));
    ...
    this.findTargetVertexAnchor = function(){
        ...
        for (var i = 0; i<registry.length; i++){
            if(!registry[i][5]){
                if(neighborSet.length == 0 && distP2(registry[i][4],this.position)>(senRange+10)){
        ...
        if(neighborAgent==this.id){// with respect to itself: Terms of type 1,2,and 3
            ...
            for (var x = this.position.x-halfStepSize*p; x<=this.position.x+halfStepSize*p; x+=stepSize){
                for(var y = this.position.y-halfStepSize*p; y<=this.position.y+halfStepSize*p; y+=stepSize){
                    ...
                    if(eventDensity > 0 && disti < senRange && disti!=0){
                        ...
                        if(p_i > 0){
                            ...
                            //constantTermW1 = eventDensity*(1-this.detectionProbabilityWithout(interestedPoint,true,neighborAgent))*(sensingDecayFactor)*p_i*p_j*areaFactor;
                            constantTerm = eventDensity*(1-this.detectionProbability(interestedPoint,true))*(sensingDecayFactor)*p_i*areaFactor/sq(disti);
                            //print(constantTerm);
                            crossDerivative = crossDerivative + constantTerm*(distiX*distiY*(sensingDecayFactor + (1/disti)));
                            secondDerivativeX = secondDerivativeX + constantTerm*(sensingDecayFactor*sq(distiX)-(sq(distiY)/disti));
                            secondDerivativeY = secondDerivativeY + constantTerm*(sensingDecayFactor*sq(distiY)-(sq(distiX)/disti));
                        }                        
                    }
                }
            }
            ...
            if(nearbyNonReflexVertices.length==0){
                ...
                for(var j = 0; j < nearbyNonReflexVertices.length; j = j+2){
                    ...
                    for(var i =0; i<pointArray.length; i++){
                        ...
                        if(distP2(s,x)!=0){
                            constantTerm = -1*sensingDecayFactor*getEventDensity(x)*(1-this.detectionProbability(x,true))*this.sensingModelFunction(x)*r*stepSize/distP2(s,x);
                        }else{
                            constantTerm = 0;
                        }
            ...
            if(arcSections.length==0){
                ...
                var constantFactor = -1*sensingDecayFactor*Math.exp(-sensingDecayFactor*senRange)*deltaTheta/senRange; 
                var s = this.position;
                ...
                for(var j = 0; j<arcSections.length; j++){// considering each arc section
                    ...
                    while(theta <= arcSections[j][1]){
                        var x = new Point2(s.x + senRange*cos(theta), s.y + senRange*sin(theta)); 
            ...
            for (var x = this.position.x-halfStepSize*p; x<=this.position.x+halfStepSize*p; x+=stepSize){
                for(var y = this.position.y-halfStepSize*p; y<=this.position.y+halfStepSize*p; y+=stepSize){
                    ...
                    if(eventDensity > 0 && disti < senRange && distj < senRange && disti!=0 && distj!=0){
                        ...
                        if(p_i>0 && p_j>0){
                            ...
                            constantTerm = eventDensity*(1-this.detectionProbabilityWithout(interestedPoint,true,neighborAgent))*(-1*sq(sensingDecayFactor))*p_i*p_j*areaFactor/(disti*distj);
    ...
    this.getJointDerivative = function(neighborAgent,modeOfNeighborAgent){//neighborAgent = j
                ...
                if(eventDensity > 0 && dist < senRange && dist!=0){
                    ...
                    if(p_i>0 && p_j>0){
                        ...
                        if(modeOfNeighborAgent==0){
                            ...
                            //print("Phi boosting!!!");
                            constantTermW1 = boostingParameterK*(1+boostingParameterY)*(sensingDecayFactor)*eventDensity*p_j*pow((1-this.detectionProbabilityWithout(interestedPoint,true,neighborAgent)),boostingParameterY+1)*pow(1-p_i,boostingParameterY)*p_i*areaFactor;
                        }
        ...
        if(nearbyNonReflexVertices.length==0){
            ...
            for(var j = 0; j < nearbyNonReflexVertices.length; j = j+2){
                ...
                for(var i =0; i<pointArray.length; i++){
                    ...
                    p_i = Math.exp(-1*sensingDecayFactor*distP2(this.position,x));
                    if(p_j>0){
                        // this is correct
                        ////constantTermW2 = getEventDensity(x)*(1-this.detectionProbability(x,true))*this.sensingModelFunctionForARay(x);
                        // this is wrong - try to avoid the following line and try to use the line above
        ...
        var constantFactor = -1*deltaTheta*senRange; // R d\theta
        var pointArrayP = []
        var constantTerm = 0;
        ...
        return ([derivativeXPart1,derivativeYPart1]);
    }
    ...
    this.getArcSections = function(){
        ...
        var arcIntersectingPoints = findObstacleIntersectingPoints(this.position);
        var nearbyNonReflexVertices = this.getNearbyNonReflexVertices();
        ...
        for(var j = 0; j < nearbyNonReflexVertices.length; j = j+2){
            ...
            var Vj = plusP2(this.position,productP2(normalizeP2(minusP2(v_j,this.position)),senRange));
        ...
        var deltaTheta = stepSizeLinear/senRange;
        var pointBefore; var pointAfter;
        var anglePairs = [];
        var arcLines = [];
        for(var j=0; j<arcAngles.length; j++){
            //print(arcAngles[j])
            arcIntersectingPoints.push(new Point2(this.position.x+senRange*cos(arcAngles[j]),this.position.y+senRange*sin(arcAngles[j])));            
            pointBefore = new Point2(this.position.x+senRange*cos(arcAngles[j]-deltaTheta),this.position.y+senRange*sin(arcAngles[j]-deltaTheta));
            pointAfter = new Point2(this.position.x+senRange*cos(arcAngles[j]+deltaTheta),this.position.y+senRange*sin(arcAngles[j]+deltaTheta));

            if(isLineOfSight(this.position,pointBefore)&&!isOutOfCanvas(pointBefore) && j==0 ){
                anglePairs.push([-Math.PI,arcAngles[j]]);
                
                var pointBeforeT = new Point2(this.position.x+senRange*cos(-Math.PI),this.position.y+senRange*sin(-Math.PI));
                ...
                var pointAfterT = new Point2(this.position.x + senRange*cos(arcAngles[j+1]),this.position.y + senRange*sin(arcAngles[j+1]));
                arcLines.push([arcIntersectingPoints[j],pointAfterT]);
            }else if(isLineOfSight(this.position,pointAfter)&&!isOutOfCanvas(pointAfter) && j==(arcAngles.length-1)){
                anglePairs.push([arcAngles[j],Math.PI]);

                var pointAfterT = new Point2(this.position.x + senRange*cos(Math.PI),this.position.y + senRange*sin(math.PI));
                arcLines.push([arcIntersectingPoints[j],pointAfterT]);
            }
