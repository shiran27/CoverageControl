...
var markedSubmodularityCandidates = []; // just to highlight some points
...
function draw() {
    ...
    if(addObstacleMode==0 && !isDebugMode && isSimulationMode && !mouseIsPressed){
        ...
        for (var i = 0; i < particleShadows.length; i++) {
            ...
            if(variableStepSizeMode && (stateUpdateIterationNumber-simulationStartIterationNumber)>50){
                
                //wait for first 3 seconds to avoid flocking
                /*var closestNeighborIndex = particleShadows[i].closestNeighbour();
                if(closestNeighborIndex == -1){// no very close by  neighbors
                    particleShadows[i].variableStepSizeUpdate(i);
                }else{
                    if(distP2(particleShadows[closestNeighborIndex].position,particleShadows[i].position)<10){
                        print('Agent '+(i+1)+' Deactivated1, N = '+(closestNeighborIndex+1));
                        particleShadows[i].updateNew(i);
                    }else if(distP2(particleShadows[closestNeighborIndex].position,particleShadows[i].position)>30){
                        particleShadows[i].variableStepSizeUpdate(i);
                    }
                    else{
                        print('Agent '+(i+1)+' Deactivated2, N = '+(closestNeighborIndex+1));
                        particleShadows[i].updateNew(i); 
                    }
                }*/
    ...
    if(submodularityMode==1){
        ...
        printPointArrayP2(markedSubmodularityCandidates,color(200,0,100),5);
        //printPoint2Array(submodularityCandidates,color(100,0,100),20);
    }else if(submodularityMode==2 || submodularityMode==2.5 ){// need to simulate the vcandidate points
