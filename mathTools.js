...
// readjust 2
function readjustCandidates2Start(){
	
	consolePrint("Modifying Candidate Points: Started. Please wait...");
	// save the existing particle shadows
	savedParticles = particleShadows;
	...
	for(var i = 1; i<N; i++){
		...
		for(var j = 0; j<submodularityCandidates.length; j++){// adding scanning candidates
			if(chosenCandidateIndexes.includes(j)){// not choses previously
				costArray[j] = 100000; // will disencourage chosing this point
			}else{
				addAgentToPoint(submodularityCandidates[j].x,submodularityCandidates[j].y);
				costArray[j] = Math.round(particleShadows[0].localObjectiveFunction()*1000)/1000;
...
function findObstacleIntersectingPoints(agentPosition){
	...
	for(var i=0; i<intersectingPoints.length; i++){
		...
		////}
		var normalDirection = normalizeP2(minusP2(intersectingPoints[i],agentPosition));
        ...
        var allignDirection = normalizeP2(minusP2(agentPosition,intersectingPoints[i]));
		...
		var inSensing1 = !isOutOfCanvas(Vj1) && isLineOfSight(agentPosition,Vj1);
		var inSensing2 = !isOutOfCanvas(Vj2) && isLineOfSight(agentPosition,Vj2);
		var inSensing3 = !isOutOfCanvas(Vj3) && isLineOfSight(agentPosition,Vj3);
